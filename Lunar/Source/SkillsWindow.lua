...
function SkillsWindow.CheckSkillForUpdate(skill)
	if oldSkillValues[skill] ~= nil and WindowData.SkillDynamicData[skill] ~= nil then
		...
		if oldSkillValue < newSkillValue then
			...
			-- table.insert(ChatSettings.ChannelColors, SystemData.ChatLogFilters.SKILL_CHANGE, {r = 0, g = 128, b = 64})
			-- table.insert(ChatSettings.ChannelColors, SystemData.ChatLogFilters.STAT_CHANGE, {r = 0, g = 128, b = 64})
			...
			if (Interface.UseNewChat) then
				...
				--PrintWStringToChatWindow(text,SystemData.ChatLogFilters.SYSTEM)
				PrintWStringToChatWindow(text,SystemData.ChatLogFilters.SKILL_CHANGE)
				--PrintWStringToChatWindow(L"text",SystemData.ChatLogFilters.SKILL_CHANGE)
				--Lunar.TestOverhead(text)
			end
			if( DoesWindowNameExist("SkillsTrackerWindow") ) then
				SkillsTracker.Update()
			end
